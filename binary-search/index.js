...
function searchNoOptimization(arr,elem
				, start /*optional start position*/
				, end /*optional end position*/){
	var index = -1;
	start = start || 0;
	...
	if(length === 1 && elem === arr[start]){
		...
		var half = Math.round(length / 2),
			middle = start+half;
		if(elem < arr[middle]){
			index = searchNoOptimization(arr,elem,start,middle);
		}else{
			index = searchNoOptimization(arr,elem,middle,end);
		}
	}
	return index;
}
module.exports = searchNoOptimization;